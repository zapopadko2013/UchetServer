...
router.get("/stock", (req, res) => {
    ...
    knex.raw(`
    select rl.createdate, rl.point, rl.company, s.product, s.attributes, s.units, rl.revisionnumber
from stockcurrent s
inner join revision_list rl
using(company,point)
inner join  
(select max(id) id from revision_list 
where company = ${company} and point = ${point}) 
rlm
on rlm.id = rl.id
inner join products pr
on s.product = pr.id and s.company = pr.company
where s.units > 0
order by s.units desc, s.product, s.attributes;
  ...
  router.get("/stocks", (req, res) => {
    ...
    //const point = req.query.point;
    knex.raw(`
    select current_timestamp as createdate, s.point, s.company, s.product, s.attributes, s.units, 
    rl.revisionnumber, ${req.userData.id} as "user"
    from stockcurrent s
    inner join products pr
    on s.product = pr.id and s.company = pr.company
    ...
    using(product)
    inner join revision_list rl
    on s.company = rl.company and s.point = rl.point
    and case 
      when rl.type = 2 then -- category
        rl.type_id = pr.category
          when rl.type = 3 then -- brand
              rl.type_id = pr.brand
                when rl.type = 4 then -- brand and category
                  rl.type_id = pr.category and rl.tag = pr.brand
                    when rl.type = 5 then -- counterparty
                      rl.type_id = c2p.counterparty
                else true
                  end
    where rl.revisionnumber = ${req.query.revisionnumber} and rl.company = ${req.userData.company}
    and s.units <> 0 and not pr.deleted
    group by s.point, s.company, s.product, s.attributes, s.units, rl.revisionnumber
    order by s.units desc, s.product, s.attributes;
...
router.post("/stock/insert", (req, res) => {
    //const company = req.userData.company;
    //const user = req.userData.id;
    knex.raw(`
    
    insert into revisiontemp
    (createdate,point,company,product,attributes,unitswas,revisionnumber,"user")
select current_timestamp, s.point, s.company, s.product, s.attributes, s.units, rl.revisionnumber, ${req.userData.id}
from stockcurrent s
inner join products pr
on s.product = pr.id and s.company = pr.company
inner join counterparty2product c2p
...
where rl.revisionnumber = ${req.body.revisionnumber} and rl.company = ${req.userData.company}
and s.units <> 0 and not pr.deleted
group by s.point, s.company, s.product, s.attributes, s.units, rl.revisionnumber
order by s.units desc, s.product, s.attributes;
  ...
  router.get("/middle/report", (req, res) => {
    const company = req.userData.company;
    //const user = req.userData.id;
    knex.raw(`
    select r.product, pr.code, pr.name, 
    r.units, r.unitswas, r.company, r.editdate, 
    attr.attributes,
    case when r.editdate is not null then 
      'passed'
      else 'not passed' end pass, 
    case when r.units = r.unitswas then 
      'match'
    ...
    left join (
      select al.listcode, 
      json_agg(json_build_object('attribute', an.values, 'value', al.value)) attributes
      from attrlist al
      inner join attributenames an
      on al.attribute = an.id
      where al.company = ${company}
      group by al.listcode
      order by al.listcode desc
      
      ) attr
      on attr.listcode = r.attributes
    where r.revisionnumber = ${req.query.revisionnumber}
  ...
  router.put("/stock/update", (req, res) => {
    ...
    knex.raw(`
      update revisiontemp
        set units = ${req.body.units}, outofrevision = 2
          where product = ${req.body.product} and attributes = ${req.body.attributes}
            and company = ${req.body.company} and point = ${req.body.point} 
              and revisionnumber = ${req.body.revisionnumber}
    `)
      .then((result) => {
        return res.status(200).json(result.rows);
      })
  ...
  router.put("/stock/add", (req, res) => {
    req.body.company = req.userData.company;
    req.body.user = req.userData.id;
    
    knex.raw("select revisiontemp_add(?)",[req.body])
      .then((result) => {
        result = result.rows[0].revisiontemp_add;
  ...
  router.delete("/stock/delete", (req, res) => {
    const company = req.userData.company;
    const isSure = req.query.sure;
    if (isSure) {
      knex.raw(`
      delete from revisiontemp
        where product = ${req.body.product} and attributes = ${req.body.attributes}
          and company = ${company} and point = ${req.body.point} 
            and revisionnumber = ${req.body.revisionnumber}
    `)
      .then((result) => {
        return res.status(200).json(result.rows);
  ...
  router.put("/add/reason", (req, res) => {
    req.body.company = req.userData.company;
    
    knex.raw("select add_reason(?)",[req.body])
      
      .then((result) => {
        return res.status(200).json(result.rows[0].add_reason);
      })
      .catch((err) => {
        console.log(err);
        return res.status(500).json(err);
  ...
  router.get("/statistics", (req, res) => {
    ...
    count(revisiontemp) total, 
    tt.passed, count(revisiontemp)-passed notpassed,
    (select count(*) 
    from revisiontemp 
    where units = unitswas 
    and revisionnumber = ${revisionnumber}
    and company = ${company}
    and point = ${point}) "matched"
    ,
    (select count(*) 
    from revisiontemp 
    where units <> unitswas 
    and revisionnumber = ${revisionnumber}
    and company = ${company}
    and point = ${point}) as nomatched
    from revisiontemp
    inner join (
    select revisionnumber, count(revisiontemp) passed
    from revisiontemp
    where units <> 0
    and company = ${company}
    and point = ${point}
    group by revisionnumber
    having revisionnumber = ${revisionnumber}
    ) tt
    using(revisionnumber)
    where revisionnumber = ${revisionnumber} 
    and company = ${company}
    and point = ${point}
    group by revisionnumber, tt.passed
    ;
    `)
      .then((result) => {
        return res.status(200).json(result.rows);
...
router.post('/revisionlist/add', (req, res) => {
  ...
  knex.raw('select revisionlist_add(?)', [req.body])
    .then((result) => {
      return res.status(200).json(result.rows[0].revisionlist_add)
    })
    .catch((err) => {
      return res.status(500).json(err)
    })
...
router.post('/revisiontemp/update', (req, res) => {
  ...
  knex.raw('select revisiontemp_update(?)', [req.body])
    .then((result) => {
      return res.status(result.rows[0].revisiontemp_update.code == "success" ? 200 : 400).json(result.rows[0].revisiontemp_update);
    })
    .catch((err) => {
      return res.status(500).json(err)
    })
...
router.post('/revisiondiary/insert', (req, res) => {
  req.body.user = req.userData.id;
  req.body.company = req.userData.company;
  helpers.serverLog(req.body);
  knex.raw('select revisiondiary_insert(?)', [req.body])
    .then((result) => {
      return res.status(200).json(result.rows[0].revisiondiary_insert)
    })
    .catch((err) => {
      return res.status(500).json(err)
    })
...
router.get("/comparetemprevision", (req, res) => {
  ...
  inner join storeprices on storeprices.stock = stockcurrent.id and  storeprices.company = stockcurrent.company
