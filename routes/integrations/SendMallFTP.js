...
function dostyk() {
	...
	var folder = './public/ftp_files/dostyk_files/';
	...
	// Параметры сервера Dostyk Plaza (id=1)
	var serverport = 21;
	var serverhost = '37.228.66.155';	
	
	console.log(d + ' Dostyk Plaza');
	
	knex.raw(`select d.ftpobject, d.ftplogin, d.ftppassword, t.company, t.point, entity2char(( xmlelement(name "ALLDATA",xmlconcat(('<OBJECT>'||d.ftpobject||'</OBJECT>')::xml,xmlelement(name "SALES",xmlagg(xmlelement(name "SALE", xmlconcat(xmlforest(t.cashbox as "PosNum",t.ticketid as "ChequeNum", to_char(t.date,'DD-MM-YYYY HH24:MI:SS') as "ChequeDate"),d.xml_task_group)))))) )::text)::xml
	FROM transactions t
	INNER JOIN (
	...
	group by d.ftpobject, d.ftplogin, d.ftppassword, t.company, t.point;`).then((result) => {
		...
		result.rows.forEach((point) => {
			...
			try {
				...
				fs.writeFileSync(folder+point.point+'('+point.company+')'+'/'+fileName, beautify('<?xml version="1.0" encoding="utf-8"?>' + point.entity2char));
			} catch (err) {
			...
			c.on('ready', function () {
				console.log('READY Dostyk Plaza('+ point.point + ')');				
				c.put(folder+point.point+'('+point.company+')'+'/'+fileName,'files/'+fileName,function (err) {
					if (err) { console.log('PUT ERROR Dostyk Plaza('+ point.point + '): ' + err); };
					//c.end();
				});
				c.end();
				console.log('RESULT Dostyk Plaza(' + point.point + '): OK');
			});
			
			c.on('error', function (err) {
				console.log('RESULT Dostyk Plaza('+ point.point +'): ' + err + ' ' + '(' + point.ftplogin + ':' + point.ftppassword + ')');
				c.end();
				return;
			});
		...
		if (!exists) { console.log('RESULT Dostyk Plaza(Нет чеков за день): OK'); }
...
function globus() {
	...
	var date_file  = ('0' + d.getDate()).slice(-2) + ('0' + (d.getMonth() + 1)).slice(-2) + year.substr(2,2);
	var date_req   = ('0' + d.getDate()).slice(-2) + '.' + ('0' + (d.getMonth() + 1)).slice(-2) + '.' + year;
	var folder = './public/ftp_files/globus_files/';
	...
	// Параметры сервера Globus (id=2)
	var serverport = 3021;
	var serverhost = '79.142.51.162';
	
	console.log(d + ' Globus');
...
xmlelement(name "Lines",xmlconcat(d.xml_task_group))))))) )::text)::xml
	...
	group by d.ftpobject, d.ftplogin, d.ftppassword, t.company, t.point;`).then((result) => {
		...
		result.rows.forEach((point) => {
			...
			c.on('ready', function () {
				console.log('READY Globus('+ point.point + ')');
				...
				c.end();
				console.log('RESULT Globus(' + point.point + '): OK');
				//console.log('');
			});
			
			c.on('error', function (err) {
				console.log('RESULT Globus('+ point.point +'): ' + err + ' ' + '(' + point.ftplogin + ':' + point.ftppassword + ')');
				c.end();
				return;
			});
		...
		if (!exists) { console.log('RESULT Globus(Нет чеков за день): OK'); }
...
xmlelement(name "Lines",xmlconcat(d.xml_task_group))))))) )::text)::xml
	...
	group by d.ftpobject, d.ftplogin, d.ftppassword, t.company, t.point;`).then((result) => {
		...
		if (!exists) { console.log('RESULT Mega(Нет чеков за день): OK'); }
...
function manual(diff) {
	...
	// Параметры сервера Dostyk Plaza (id=1)
	var serverport = 3021;
	//var serverhost = 'salesflow.tsd.kz';
	var serverhost = 'asspmca.mega.kz'; //Новый адрес сервера АССП: 2.78.60.189, DNS: asspmca.mega.kz
	
	console.log(d + ' Manual');
...
xmlelement(name "Lines",xmlconcat(d.xml_task_group))))))) )::text)::xml
	...
	group by d.ftpobject, d.ftplogin, d.ftppassword, t.company, t.point;`).then((result) => {
		...
		result.rows.forEach((point) => {
			...
			c.on('ready', function () {
				console.log('READY Manual('+ point.point + ')');
				c.put(folder+point.point+'('+point.company+')'+'/'+fileName,fileName,function (err) {
					//if (err) { console.log('PUT err : ' + err); };
					if (err) { console.log('PUT ERROR Manual('+ point.point + '): ' + err); };
					//c.end();
				});
				c.end();
				console.log('RESULT Manual(' + point.point + '): OK');
				console.log(date_req + ' ' + fileName);
			});
			
			c.on('error', function (err) {
				console.log('RESULT Manual('+ point.point +'): ' + err + ' ' + '(' + point.ftplogin + ':' + point.ftppassword + ')');
				c.end();
				return;
			});
		...
		if (!exists) { console.log('RESULT Manual(Нет чеков за день): OK'); }
...
xmlelement(name "Lines",xmlconcat(d.xml_task_group))))))) )::text)::xml
	...
	group by d.ftpobject, d.ftplogin, d.ftppassword, t.company, t.point;`).then((result) => {
		...
		result.rows.forEach((point) => {
			...
			c.on('ready', function () {
				console.log('READY Manual3('+ point.point + ')');
				c.put(folder+point.point+'('+point.company+')'+'/'+fileName,fileName,function (err) {
					//if (err) { console.log('PUT err : ' + err); };
					if (err) { console.log('PUT ERROR Manual3('+ point.point + '): ' + err); };
					//c.end();
				});
				c.end();
				console.log('RESULT Manual3(' + point.point + '): OK');
				//console.log('');
			});
			
			c.on('error', function (err) {
				console.log('RESULT Manual3('+ point.point +'): ' + err + ' ' + '(' + point.ftplogin + ':' + point.ftppassword + ')');
				c.end();
				return;
			});
